...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.verify_password(form.password.data) and user.isDisabled() == False:
            login_user(user, form.remember_me.data)
            return redirect(request.args.get('next') or url_for('main.viewBlocks'))
...
def register():
    ...
    if form.validate_on_submit():
        user = User(email=form.email.data,
                    password=form.password.data, lName = form.lName.data, fName = form.fName.data, confirmed= 0)
        db.session.add(user)
        db.session.flush()
        db.session.commit()
